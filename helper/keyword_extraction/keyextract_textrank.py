...
def getKeywords_tr(path_out, data, path_stop, topK, pos):
    ...
    result = pd.DataFrame({'label': labels, "content": contents, "textrank_keys": keys}, columns=['label','content', 'textrank_keys'])
    return result



def textrank_getKeywords(path_in,path_out,path_stop,topk,pos=['n','v','vn','a','vd']):
    df = pd.read_csv(path_in)
    # textrank关键词抽取
    text = ' '.join(df['content'].tolist())
    jieba.analyse.set_stop_words(path_stop)  # 加载自定义停用词表
    keywords = jieba.analyse.textrank(text,topk, allowPOS=pos)  # TextRank关键词提取，词性筛选
    df_toSave = pd.DataFrame({'textrank_keys': keywords}, columns=['textrank_keys'])
    df_toSave.to_csv(path_out, index=False)
    return keywords
    # getKeywords_tr(path_out, df, path_stop,pos)
